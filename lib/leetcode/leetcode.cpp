...
void PrintMs() {
    ...
    if (last == std::chrono::high_resolution_clock::time_point{}) {
        last = current;
        i++;
        return;
    }

    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(current - last);
    double result = duration.count();

    i++;
    std::cout << "间隔：" << result << "ms" << std::endl;
}
